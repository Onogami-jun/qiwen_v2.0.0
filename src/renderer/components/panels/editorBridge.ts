...
export function flashEditorChange(): void {
  ...
  if (!el) return;
  var savedEl = el;
  savedEl.style.transition = 'box-shadow .2s ease-out';
  savedEl.style.boxShadow = '0 0 0 4px var(--accent, #c8a96e)';
  if (_ft) clearTimeout(_ft);
  _ft = setTimeout(function() { savedEl.style.boxShadow = ''; _ft = null; }, 800);
}
...
export function scrollToChange(): void {
  ...
  if (sel && sel.rangeCount) {
    ...
    if (rect.top < 0 || rect.bottom > window.innerHeight) savedEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
}
