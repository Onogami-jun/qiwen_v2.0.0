 ...
 * Clean JSX — zero template literals, zero backticks.
 */
...
function parseTags(content: string): { pure: string; plan: { title: string; steps: AgentStep[] } | null; thinking: string | null; actions: ParsedAction[] } {
  ...
  if (pm) {
    ...
    for (const m of inner.matchAll(/<step\s+id="(\d+)"[^>]*>([\s\S]*?)<\/step>/g)) {
...
function renderMD(text: string): string {
  ...
  h = h.replace(/```(\w*)\n([\s\S]*?)```/g, function(_0: string, _1: string, code: string) {
...
const SendIcon: React.FC = () => (
  <svg className="pn-chat__send-icon" viewBox="0 0 16 16" fill="none">
    <path d="M2 2l12 5.5L2 14l3-6.5L2 2z" fill="currentColor" stroke="currentColor" strokeWidth="1" strokeLinejoin="round"/>
  </svg>
);
...
/* ── Thinking Block — typewriter + auto-collapse ──────────── */
const ThinkingBlock: React.FC<{ content: string }> = ({ content }) => {
  ...
  useEffect(() => {
    if (prev.current !== content) { setPos(0); setOpen(true); prev.current = content; }
  }, [content]);
  useEffect(() => {
    if (done || !open) return;
    const t = setTimeout(function() { setPos(function(p: number) { return Math.min(p + 5, content.length); }); }, 10);
    return function() { clearTimeout(t); };
  }, [pos, content.length, done, open]);
  useEffect(() => {
    if (!done || !open) return;
    const t = setTimeout(function() { setOpen(false); }, 4000);
    return function() { clearTimeout(t); };
  }, [done, open]);
...
const PlanCard: React.FC<{ title: string; steps: AgentStep[]; onStart: () => void }> = ({ title, steps, onStart }) => (
  <div className="pn-plan-card">
    <div className="pn-plan-card__title">{'📋 ' + title}</div>
    {steps.map(function(s: AgentStep) {
      return (
        <div key={s.id} className="pn-plan-card__step" style={{ opacity: s.status === 'done' ? 0.5 : 1 }}>
          <span className="pn-plan-card__dot" style={{
    ...
    <button className="pn-plan-card__btn" onClick={onStart}>开始执行</button>
  </div>
);
...
const MsgBubble: React.FC<BubbleProps> = ({ msg, isLast, onPlanStart, onAccept, onReject, pendingId }) => {
  ...
  const time = (function() { try { return new Date(msg.createdAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }); } catch (_e) { return ''; } })();
  const actions: any[] = useMemo(function() { return (meta && meta.actions) || []; }, [meta]);
  ...
  return (
    <div className={'pn-chat-msg pn-chat-msg--' + msg.role}>
      ...
      <div style={{ minWidth: 0 }}>
        ...
        {!isU && meta && meta.plan && isLast && (
          <PlanCard title={meta.plan.title} steps={meta.plan.steps} onStart={onPlanStart}/>
        )}
        {!isU && actions.filter(function(a: any) { return getSafety(a.type) === 'confirm'; }).map(function(a: any, i: number) {
          return <ActionConfirm key={a.type + '-' + i} action={a} pending={pendingId != null}
            onAccept={function() { onAccept(a); }} onReject={function() { onReject(a); }}/>;
        })}
        {!isU && meta && meta.actionResults && (meta.actionResults as string[]).map(function(r: string, i: number) {
          return <div key={String(i)} className="pn-action-ok"><span>{'✓'}</span><span>{r}</span></div>;
        })}
        {(meta && meta.pureContent) || msg.content ? (
          <div className="pn-chat-msg__bubble">
            {isU ? msg.content : <div dangerouslySetInnerHTML={{ __html: renderMD((meta && meta.pureContent) || msg.content) }}/>}
          </div>
        ) : null}
...
const ChatPanel: React.FC<Props> = ({ node, getDocumentContent }) => {
  const docId: string | null = useSelector(function(s: RootState) { return ((s as any).panelLayout && (s as any).panelLayout.loadedDocumentId) || null; }) || null;
  ...
  const persist = useCallback(async function(m: ChatMessage) { try { await ipc.invoke('db:saveChatMessage', m); } catch (_e) { /* ok */ } }, []);

  /* ── AI Call ─────────────────────────────────────────────── */
  const callAi = useCallback(async function(toSend: ChatMessage[]): Promise<string> {
    setStreaming(true); setCbState(function(p: ControlBarState) { return { visible: true, status: 'thinking' as const, currentStep: p.currentStep, totalSteps: p.totalSteps, completedSteps: p.completedSteps }; });
    try {
      ...
      setCbState(function(p: ControlBarState) { return { visible: true, status: 'executing' as const, currentStep: p.currentStep, totalSteps: p.totalSteps, completedSteps: p.completedSteps }; });
      return content;
  ...
  var execActions = useCallback(function(actions: ParsedAction[]): string[] {
    ...
    for (var i = 0; i < actions.length; i++) {
      ...
      setCbState(function(p: ControlBarState) { return { visible: true, status: 'executing' as const, currentStep: r.message, totalSteps: p.totalSteps, completedSteps: p.completedSteps }; });
    }
  ...
  function processResponse(raw: string): ChatMessage {
    ...
    var pure = parsed.pure, plan = parsed.plan, thinking = parsed.thinking, actions = parsed.actions;
    ...
    if (executable.length) {
      ...
      setCbState(function(p: ControlBarState) { return { visible: true, status: 'executing' as const, currentStep: p.currentStep, totalSteps: p.totalSteps, completedSteps: p.completedSteps + executable.length }; });
    }
  ...
  var startPlan = useCallback(async function() {
    ...
    try { var content2 = await callAi(cur); var am2 = processResponse(content2); setMsgs(function(p: ChatMessage[]) { return p.concat([am2]); }); persist(am2); if (!autoRef.current) setCbState({ visible: false, status: 'idle' as const, currentStep: '', totalSteps: 0, completedSteps: 0 }); } catch (_e) { /* ok */ }
  ...
  var acceptAction = useCallback(async function(action: ParsedAction) {
    ...
    try { var content3 = await callAi(cur); var am3 = processResponse(content3); setMsgs(function(p: ChatMessage[]) { return p.concat([am3]); }); persist(am3); } catch (_e) { /* ok */ }
  ...
  var rejectAction = useCallback(async function(_action: ParsedAction) {
    ...
    var cur = msgsRef.current.concat([fb]); setMsgs(cur); persist(fb);
    try { var content4 = await callAi(cur); var am4 = processResponse(content4); setMsgs(function(p: ChatMessage[]) { return p.concat([am4]); }); persist(am4); } catch (_e) { /* ok */ }
  }, [docId, persist, callAi]);
  ...
  var handleControlOverride = useCallback(async function(instr: string) {
    ...
    var cur = msgsRef.current.concat([cm]); setMsgs(cur); persist(cm);
    try { var content5 = await callAi(cur); var am5 = processResponse(content5); setMsgs(function(p: ChatMessage[]) { return p.concat([am5]); }); persist(am5); setCbState(function(p: ControlBarState) { return { visible: true, status: 'executing' as const, currentStep: p.currentStep, totalSteps: p.totalSteps, completedSteps: p.completedSteps }; }); } catch (_e) { /* ok */ }
  }, [docId, persist, callAi]);
  ...
  return (
    <Panel node={node}>
      <div className="pn-chat">
        {/* Auto-mode bar */}
        {autoMode && (
          <div style={{ display:'flex',alignItems:'center',gap:8,padding:'6px 14px',background:'rgba(200,169,110,0.1)',borderBottom:'1px solid var(--accent,#c8a96e)',fontSize:12,color:'var(--accent,#c8a96e)' }}>
            <span style={{ width:6,height:6,borderRadius:'50%',background:'var(--accent,#c8a96e)',animation:'pulse 1.5s infinite' }}/>
            <span style={{ flex:1,fontWeight:500 }}>{'🤖 自动模式 — AI 自主执行中'}</span>
            <button onClick={function() { setAutoMode(false); }} style={{ padding:'2px 10px',borderRadius:12,border:'1px solid var(--accent,#c8a96e)',background:'transparent',color:'var(--accent,#c8a96e)',cursor:'pointer',fontSize:11 }}>停止</button>
          </div>
        )}

        {/* Empty state */}
        {msgs.length === 0 ? (
          <div className="pn-chat__empty">
            <EmptyIcon/>
            <p className="pn-chat__empty-text">
              告诉我你想写什么<br/>比如"帮我写一篇产品发布公告"
            </p>
          </div>
        ) : (
            ...
            {streaming && (
              <div className="pn-chat-streaming">
                ...
                <span className="pn-chat-streaming__dots">
                  <span className="pn-chat-streaming__dot"/>
                  <span className="pn-chat-streaming__dot"/>
                  <span className="pn-chat-streaming__dot"/>
                </span>
              </div>
        ...
        <div className="pn-chat__input-area">
          <textarea className="pn-chat__input" value={input} onChange={onInputC} onKeyDown={onKeyDown}
            placeholder={autoMode ? '自动模式运行中…' : '描述你的写作需求…'}
            rows={1} disabled={streaming || autoMode}/>
          <button className="pn-chat__send" onClick={send} disabled={streaming || autoMode || !input.trim()} title="发送">
            <SendIcon/>
          </button>
        </div>

        {/* Control bar */}
        <AgentControlBar state={cbState}
          onStop={function() { setAutoMode(false); setCbState({ visible: false, status: 'idle', currentStep: '', totalSteps: 0, completedSteps: 0 }); }}
          onOverride={handleControlOverride}/>
      </div>
    </Panel>
  );
};
