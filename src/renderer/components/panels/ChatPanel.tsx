...
const AP = ['你是启文（QiWen Writer）的内置 AI Agent。你可以直接操作编辑器来完成写作任务。','','## 工作流程','1. 需求不清晰时先提问（最多3个）→ 生成任务计划','2. 使用 action 标签操作编辑器 → 汇报结果','3. 每步完成后等待用户确认（除非用户开启自动模式）','','## 可用 Action','- append: 末尾追加（自动）','- insert: 光标插入（自动）','- replace: 替换（确认）','- rewrite: 改写（确认）','- delete: 删除（确认）','','## 重要','- 安全操作直接执行并告知结果','- 修改操作展示 diff 等用户确认','- 用户说"自动模式"→ 全自主执行直到叫停'].join('\n');
...
const PlanCard: React.FC<{title:string;steps:AgentStep[];onStart:()=>void}> = function(p:{title:string;steps:AgentStep[];onStart:()=>void}) {
  ...
  return (<div className="pn-plan-card"><div className="pn-plan-card__title">{'📋 '+title}</div>{steps.map(function(s:AgentStep){return(<div key={s.id} className="pn-plan-card__step" style={{opacity:s.status==='done'?0.5:1}}><span className="pn-plan-card__dot" style={{background:s.status==='done'?'var(--color-success,#22c55e)':s.status==='doing'?'var(--accent,#c8a96e)':'var(--border,#e2e5e9)',color:s.status!=='pending'?'#fff':'var(--text-tertiary)'}}>{s.status==='done'?'?':s.status==='doing'?'?':s.id}</span><span style={{color:'var(--text-secondary)',textDecoration:s.status==='done'?'line-through':'none'}}>{s.title}</span></div>)})}<button className="pn-plan-card__btn" onClick={onStart}>开始执行</button></div>);
};
...
const ChatPanel: React.FC<Props> = function(p:Props) {
  ...
  var callAiStream = useCallback(async function(toSend:ChatMessage[]):Promise<string> {
    ...
    return new Promise<string>(function(resolve,reject){
      (async function(){try{var doc=(getDocumentContent&&getDocumentContent())||'';var pref=await buildSystemPrompt();var sysParts=[pref,AP];if(doc)sysParts.push('\n当前文档：\n'+doc.slice(0,3000));if(autoRef.current)sysParts.push('\n用户已开启自动模式，自主执行无需确认。');var recent=toSend.slice(-25).map(function(m:ChatMessage){return{role:m.role,content:m.content}});var messages=[{role:'system',content:sysParts.filter(Boolean).join('\n')},...recent];var onChunk=function(data:{content:string;full:string}){Bridge.showEditorBanner('AI 正在生成回复…')};if(window.electronAPI&&window.electronAPI.onStreamChunk){window.electronAPI.onStreamChunk(onChunk)}try{var fullContent=await ipc.invoke<string>('ai:chat-stream-v2',{messages:messages,apiKey:'',model:''});Bridge.hideEditorBanner();setCbState(function(p:ControlBarState):ControlBarState{return{visible:true,status:'executing' as const,currentStep:p.currentStep,totalSteps:p.totalSteps,completedSteps:p.completedSteps}});resolve(fullContent)}finally{if(window.electronAPI&&window.electronAPI.removeStreamChunk){window.electronAPI.removeStreamChunk(onChunk)}}}catch(err:any){Bridge.hideEditorBanner();reject(err)}finally{setStreaming(false)}})()})},[getDocumentContent]);

  var execActions=useCallback(function(actions:ParsedAction[]):string[]{var results:string[]=[];for(var i=0;i<actions.length;i++){var a=actions[i];if(getSafety(a.type)!=='safe'&&!autoRef.current)continue;var r:Bridge.ActionResult={success:false,message:''};Bridge.showEditorBanner('正在'+(a.type==='append'?'追加内容':a.type==='insert'?'插入内容':a.type==='replace'?'替换文本':a.type==='rewrite'?'改写段落':a.type==='delete'?'删除内容':'操作中')+'…');var st=Date.now();switch(a.type){case'append':r=Bridge.actionAppend(a.payload.title||'',a.content);break;case'insert':r=Bridge.actionInsert(a.content);break;case'replace':r=Bridge.actionReplace(a.payload.target||'',a.content);break;case'rewrite':r=Bridge.actionRewrite(a.payload.target||'',a.content);break;case'delete':r=Bridge.actionDelete(a.payload.target||'');break}var elapsed=Date.now()-st;if(elapsed<600)setTimeout(function(){Bridge.hideEditorBanner()},600-elapsed);else Bridge.hideEditorBanner();if(r.message)results.push(r.success?r.message:'FAIL:'+r.message);setCbState(function(p:ControlBarState):ControlBarState{return{visible:true,status:'executing' as const,currentStep:r.message,totalSteps:p.totalSteps,completedSteps:p.completedSteps}})}return results},[]);
  ...
  var send=useCallback(async function(){var t=input.trim();if(!t||streaming||!docId)return;setInput('');if(t.indexOf('自动模式')>=0||t.toLowerCase().indexOf('auto mode')>=0)setAutoMode(true);if(t.indexOf('停止自动')>=0||t.indexOf('取消自动')>=0||t.indexOf('手动模式')>=0){setAutoMode(false);setCbState({visible:false,status:'idle',currentStep:'',totalSteps:0,completedSteps:0})}var um:ChatMessage={id:msgId(),documentId:docId,role:'user',content:t,createdAt:new Date().toISOString()};var cur=msgsRef.current.concat([um]);setMsgs(cur);persist(um);try{var content=await callAiStream(cur);var am=processResponse(content);setMsgs(function(p:ChatMessage[]){return p.concat([am])});persist(am);if(autoRef.current&&(am.meta as any)&&(am.meta as any).plan)setTimeout(function(){autoContinue(am)},1200);else if(!autoRef.current)setCbState({visible:false,status:'idle',currentStep:'',totalSteps:0,completedSteps:0})}catch(err:any){setMsgs(function(p:ChatMessage[]){return p.concat([{id:msgId(),documentId:docId,role:'assistant',content:'FAIL:'+((err&&err.message)||'unknown'),createdAt:new Date().toISOString(),meta:{kind:'normal',pureContent:'FAIL:'+((err&&err.message)||'unknown')}}])});setCbState({visible:false,status:'idle',currentStep:'',totalSteps:0,completedSteps:0})}},[input,streaming,docId,persist,callAiStream]);

  var autoContinue=useCallback(async function(lastMsg:ChatMessage){if(!autoRef.current||!docId)return;var cm:ChatMessage={id:msgId(),documentId:docId,role:'user',content:'继续下一步（自动模式）',createdAt:new Date().toISOString()};var cur=msgsRef.current.concat([cm]);setMsgs(cur);persist(cm);try{var content=await callAiStream(cur),am=processResponse(content);setMsgs(function(p:ChatMessage[]){return p.concat([am])});persist(am);if(autoRef.current){var mp=(am.meta as any)&&(am.meta as any).plan,ad=!mp||((mp.steps||[]) as AgentStep[]).every(function(s:AgentStep){return s.status==='done'});if(ad){setAutoMode(false);setCbState({visible:false,status:'idle',currentStep:'',totalSteps:0,completedSteps:0})}else setTimeout(function(){autoContinue(am)},1500)}}catch(_e){setAutoMode(false);setCbState({visible:false,status:'idle',currentStep:'',totalSteps:0,completedSteps:0})}},[docId,callAiStream,persist]);
  var startPlan=useCallback(async function(){if(streaming||!docId)return;var cm:ChatMessage={id:msgId(),documentId:docId,role:'user',content:'开始执行计划',createdAt:new Date().toISOString()};var cur=msgsRef.current.concat([cm]);setMsgs(cur);persist(cm);var lm=msgsRef.current.length>0?((msgsRef.current[msgsRef.current.length-1]||{}).meta as any):null,plan=lm&&lm.plan;if(plan)setCbState({visible:true,status:'executing' as const,currentStep:(plan.steps[0]&&plan.steps[0].title)||'',totalSteps:plan.steps.length,completedSteps:0});try{var c2=await callAiStream(cur),a2=processResponse(c2);setMsgs(function(p:ChatMessage[]){return p.concat([a2])});persist(a2);if(!autoRef.current)setCbState({visible:false,status:'idle' as const,currentStep:'',totalSteps:0,completedSteps:0})}catch(_e){}},[streaming,docId,persist,callAiStream]);
  var acceptAction=useCallback(async function(action:ParsedAction){setPendingActionId(action.type+'-'+Date.now());var r:Bridge.ActionResult={success:false,message:''};switch(action.type){case'replace':r=Bridge.actionReplace(action.payload.target||'',action.content);break;case'rewrite':r=Bridge.actionRewrite(action.payload.target||'',action.content);break;case'delete':r=Bridge.actionDelete(action.payload.target||'');break}setPendingActionId(null);var fb:ChatMessage={id:msgId(),documentId:docId!,role:'user',content:'操作已完成：'+r.message+'。请继续下一步。',createdAt:new Date().toISOString()};var cur=msgsRef.current.concat([fb]);setMsgs(cur);persist(fb);try{var c3=await callAiStream(cur),a3=processResponse(c3);setMsgs(function(p:ChatMessage[]){return p.concat([a3])});persist(a3)}catch(_e){}},[docId,persist,callAiStream]);
  var rejectAction=useCallback(async function(_action:ParsedAction){var fb:ChatMessage={id:msgId(),documentId:docId!,role:'user',content:'我拒绝了那个操作。请提供替代方案。',createdAt:new Date().toISOString()};var cur=msgsRef.current.concat([fb]);setMsgs(cur);persist(fb);try{var c4=await callAiStream(cur),a4=processResponse(c4);setMsgs(function(p:ChatMessage[]){return p.concat([a4])});persist(a4)}catch(_e){}},[docId,persist,callAiStream]);
  var handleControlOverride=useCallback(async function(instr:string){var cm:ChatMessage={id:msgId(),documentId:docId!,role:'user',content:'（打断当前任务）'+instr,createdAt:new Date().toISOString()};var cur=msgsRef.current.concat([cm]);setMsgs(cur);persist(cm);try{var c5=await callAiStream(cur),a5=processResponse(c5);setMsgs(function(p:ChatMessage[]){return p.concat([a5])});persist(a5);setCbState(function(p:ControlBarState):ControlBarState{return{visible:true,status:'executing' as const,currentStep:p.currentStep,totalSteps:p.totalSteps,completedSteps:p.completedSteps}})}catch(_e){}},[docId,persist,callAiStream]);
