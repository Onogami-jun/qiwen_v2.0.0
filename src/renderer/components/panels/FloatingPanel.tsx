...
function checkSnap(mx: number, my: number): { panelId: string; edge: DropEdge } | null {
  let bestPanel = ''; let bestEdge: DropEdge = 'left'; let bestDist = Infinity;
  document.querySelectorAll('[data-panel-id]').forEach(el => {
    ...
    if (Math.abs(ds[mi]) <= SNAP && ds[mi] < bestDist) {
      bestDist = ds[mi]; bestEdge = edges[mi]; bestPanel = (el as HTMLElement).dataset.panelId ?? '';
    }
  });
  return bestPanel ? { panelId: bestPanel, edge: bestEdge } : null;
}
...
const FloatingPanel: React.FC<Props> = ({ fp, editorChildren, getDocContent }) => {
  ...
  const onTitleDown = useCallback((e: React.MouseEvent) => {
    ...
    dr.current = { sx: e.clientX, sy: e.clientY, x: fp.x, y: fp.y, on: true };
    const mv = (ev: MouseEvent) => {
      if (!dr.current.on) return;
      dispatch(moveFloating({ id: fp.id, x: ev.clientX - dr.current.sx + dr.current.x, y: ev.clientY - dr.current.sy + dr.current.y }));
    };
  ...
  const onEdgeDown = useCallback((e: React.MouseEvent) => {
    ...
    const mv = (ev: MouseEvent) => {
      ...
      const { sx, sy, w, h, x, y, dir: d } = rr.current;
      const dx = ev.clientX - sx, dy = ev.clientY - sy;
      let nw = w, nh = h, nx = x, ny = y;
      if (d === 'e' || d === 'ne' || d === 'se') nw = Math.max(250, w + dx);
      if (d === 'w' || d === 'nw' || d === 'sw') { nw = Math.max(250, w - dx); nx = x + (w - nw); }
      if (d === 's' || d === 'se' || d === 'sw') nh = Math.max(150, h + dy);
      if (d === 'n' || d === 'ne' || d === 'nw') { nh = Math.max(150, h - dy); ny = y + (h - nh); }
      if (nw !== w || nh !== h) dispatch(resizeFloating({ id: fp.id, width: nw, height: nh }));
      if (nx !== x || ny !== y) dispatch(moveFloating({ id: fp.id, x: nx, y: ny }));
    };
  ...
  const onMove = useCallback((e: React.MouseEvent) => {
    ...
    (e.target as HTMLElement).style.cursor = (d && CURSORS[d]) || 'auto';
  }, []);
