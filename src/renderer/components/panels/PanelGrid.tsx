...
import type { PanelNode, LeafPanel } from './types';
...
export function renderPanelNode(
  node: PanelNode, path: string,
  editorChildren?: React.ReactNode,
  getDocContent?: () => string,
): React.ReactNode {
  if (node.type === 'split') {
    return <SplitPane node={node} path={path} editorChildren={editorChildren} getDocContent={getDocContent} />;
  }
  const leaf = node as LeafPanel;
  if (leaf.panelType === 'chat') {
    return <ChatPanel node={leaf} getDocumentContent={getDocContent} />;
  }
...
const PanelGrid: React.FC<Props> = ({ documentId, children, getDocumentContent }) => {
  ...
  // Persist (debounced)
  const persist = useCallback(async (t: PanelNode) => {
    try { await ipc.invoke('db:savePanelLayout', { documentId, layoutJson: JSON.stringify(t) }); } catch {}
  }, [documentId]);
  useEffect(() => {
    if (tree && documentId) {
      const tm = setTimeout(() => persist(tree), 500);
      return () => clearTimeout(tm);
    }
  }, [tree, documentId, persist]);
